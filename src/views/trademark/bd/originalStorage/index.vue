...
<script setup>
import OriginalStorageForm from './form.vue'
import { originalStorageAdd, originalStorageEdit, originalStorageDel, originalStorageGetList, originalStorageSearch } from '@/api/trademark/bd/originalStorage'
...
function handleGetTable () {
  ...
  originalStorageSearch({ ...formInline.value, page: queryOptions.value.page, pageSize: queryOptions.value.pageSize }).then(res => {
    list.value = res.data.list
    tableOptions.value.total = res.data.total
  }).catch(() => {
    originalStorageGetList(queryOptions.value).then(res => {
      list.value = res.data.list
      tableOptions.value.total = res.data.total
    })
  }).finally(() => {
...
function handleDelete (item) {
  originalStorageDel({ id: item.id }).then(() => {
