...
<script setup>
import BrandCategoryForm from './form.vue'
import { brandCategoryAdd, brandCategoryEdit, brandCategoryDel, brandCategoryGetList, brandCategorySearch } from '@/api/trademark/bd/brandCategory'
...
function handleGetTable () {
  ...
  brandCategorySearch({ ...formInline.value, page: queryOptions.value.page, pageSize: queryOptions.value.pageSize }).then(res => {
    list.value = res.data.list
    tableOptions.value.total = res.data.total
  }).catch(() => {
    brandCategoryGetList(queryOptions.value).then(res => {
      list.value = res.data.list
      tableOptions.value.total = res.data.total
    })
  }).finally(() => {
...
function handleDelete (item) {
  brandCategoryDel({ id: item.id }).then(() => {
