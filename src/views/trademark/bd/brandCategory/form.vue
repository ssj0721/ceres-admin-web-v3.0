...
<script setup>
import { brandCategoryAdd, brandCategoryEdit } from '@/api/trademark/bd/brandCategory'
...
const formRules = {
  type: [
    { required: true, message: '请输入品牌类别', trigger: 'blur' },
  ],
}
...
watch(item, () => {
  if (item.value?.id) {
    dialogOption.value.title = '修改品牌类别'
    form.value = { ...item.value }
  } else {
    dialogOption.value.title = '新增品牌类别'
    handleResetForm()
  }
}, { deep: true })
...
function handleConfirm () {
  formRef.value.validate(async (val) => {
    ...
    try {
      if (form.value.id) {
        await brandCategoryEdit(form.value)
      } else {
        await brandCategoryAdd(form.value)
      }
