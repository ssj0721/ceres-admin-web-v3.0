...
<script setup>
import { companyAddressAdd, companyAddressEdit } from '@/api/trademark/bd/companyAddress'
...
watch(item, () => {
  if (item.value?.id) {
    dialogOption.value.title = '修改公司地址'
    form.value = { ...item.value }
  } else {
    dialogOption.value.title = '新增公司地址'
    handleResetForm()
  }
}, { deep: true })
...
function handleConfirm () {
  formRef.value.validate(async (val) => {
    ...
    try {
      if (form.value.id) {
        await companyAddressEdit(form.value)
      } else {
        await companyAddressAdd(form.value)
      }
