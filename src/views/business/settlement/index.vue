<template>
  ...
  <div class="pending">
    ...
    <el-dialog
      v-model="addVisible"
      :title="shopDetails.checkState==0?'供应商入驻审核':'供应商入驻详情'"
      :align-center="true"
      width="60%"
      center
    ...
    >
      <div v-loading="loading.detail">
        ...
        <el-card>
          <el-descriptions title="供应商信息" :column="1">
            <el-descriptions-item label="供应商名称：">{{ shopDetails.shopName }}</el-descriptions-item>
            <el-descriptions-item label="供应商类型：">{{ shopDetails.authenType==1?'个人':shopDetails.authenType==2?'个体工商户':shopDetails.authenType==3?'企业':shopDetails.authenType==4?'其他组织':'-' }}</el-descriptions-item>
            <el-descriptions-item label="客服电话：">{{ shopDetails.shopName }}</el-descriptions-item>
            <el-descriptions-item label="供应商负责人：">{{ shopDetails.chargePersonName }}</el-descriptions-item>
            <el-descriptions-item label="负责人电话：">{{ shopDetails.chargePersonPhone }}</el-descriptions-item>
            <el-descriptions-item label="供应商地址：">{{ shopDetails.shopAdress }}</el-descriptions-item>
          </el-descriptions>
        </el-card>
        <el-card>
    ...
    <el-dialog
      ...
      @closed="checkClosed"
    >
      <el-form ref="checkRef" :model="checkForm" :rules="rules" label-width="90px">
        ...
        <el-form-item v-if="checkForm.checkHandle==1" label="联营商" prop="orgcode">
          <el-select v-model="checkForm.orgcode" placeholder="请选择联营商">
...
import { settlementGetAll, settlementDelete, settlementHandle, usernameSelect } from '@/api/business'
...
let searchData = ref({
  shopName: '', // 供应商名称
  shopPhone: '', // 注册手机号
  authenType: '', // 供应商类型 1-个人 2-个体工商户 3-企业 4-其他组织
  dates: [], // 时间数组
...
const rules = ref({
  ...
  orgcode: [
    { required: true, message: '请选择联营商', trigger: 'change' },
  ]
})
  ...
  formEl.validate((valid) => {
    if (valid) {
      ...
      const params = {
        ...
        orgcode: checkForm.value.orgcode
      }
