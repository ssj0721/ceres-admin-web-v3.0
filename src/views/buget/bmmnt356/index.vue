...
<template>
  <basic-container>
    <div style="width: 100%; display: flex; font-size: large;">&nbsp;ScmAdm025</div>
...
<script>
import { bmmnt356Api } from "@/views/buget/bmmnt356/index.js";
...
export default {
  ...
  data() {
    ...
    return {
      subPath: "bm/tmp/bmmnt356" + year + month, //存储路径
      uploadFileList: [],
      visible: false,
      excelConfig: {
        uploadData: {
  ...
  created() {
    //初始化权限变量
    this.$viewUtil.initView(this, "bmmnt356", "split");
    this.initData();
  ...
  methods: {
    ...
    async exportExcel(row) {
      ...
      //     this.$dlg.Loading(false);
      //   });
      const res =   await bmmnt356Api.exportExcel(params)
      if (res.code === 0) {
        this.$dlg.Loading(false);
    ...
    async onConfirm(fileList) {
      ...
      //   });
      const res = await bmmnt356Api.importExcel(params)
      if (res.code === 0) {
        let msg = "";
            if (
    ...
    //     });
    const res =   await bmmnt356Api.query(params)
        // .then((res) => {
          if (res.code === 0) {
            let data = res.result;
            this.crudData = data.tableData;
    ...
    async split(row) {
      ...
      //     console.log("error", res);
      //   });
      const res =   await bmmnt356Api.split(params)
           let state = res.code;
          if (state === 0) {
            this.$message.success("操作成功");
            await this.queryList();
    ...
    approve(row) {
      ...
      this.$confirm("确定要确认选择的资料？", "提示", {
        ...
        .then(() => {
          bmmnt356Api.approve(params).then((res) => {
            if (res.data.code === 0) {
