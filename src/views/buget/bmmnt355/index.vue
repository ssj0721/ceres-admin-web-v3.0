...
<template>
  <basic-container>
    <div style="width: 100%; display: flex; font-size: large;">&nbsp;ScmAdm024</div>
    <el-row :gutter="5" style="margin-bottom: 10px">
      <el-col :span="1.5">
        <el-button v-if="permissions.query" type="primary" @click="queryList(true)">
          {{ $t(`Action.Query`)}}
        </el-button></el-col>
      <el-col :span="1.5">
...
<script>
import { bmmnt355Api } from "@/views/buget/bmmnt355/index.js";
...
export default {
  ...
  data() {
    ...
    return {
      subPath: "bm/tmp/bmmnt355" + year + month, //存储路径
      uploadFileList: [],
      visible: false,
      excelConfig: {
        uploadData: {
  ...
  created() {
    //初始化权限变量
    this.$viewUtil.initView(this, "bmmnt355", "split");
    this.initData();
    ...
    // if (local1) {
    //   this.permissions = JSON.parse(local1);
  ...
  methods: {
    ...
    //       this.$dlg.Loading(false);
    //     });
    const res =   await bmmnt355Api.exportExcel(params)
    if (res.code === 0) {
        this.$dlg.Loading(false);
    ...
    async onConfirm(fileList) {
      ...
      const res = await bmmnt355Api.importExcel(params)
      if (res.code === 0) {
        let msg = "";
            if (
    ...
    //     });
        const res =   await bmmnt355Api.query(params)
        // .then((res) => {
          if (res.code === 0) {
            let data = res.result;
            this.crudData = data.tableData;
    ...
    //       console.log("error", res);
    //     });
        const res =   await bmmnt355Api.split(params)
    ...
    approve(row) {
      ...
      this.$confirm("确定要确认选择的资料？", "提示", {
        ...
        .then(() => {
          bmmnt355Api.approve(params).then((res) => {
            if (res.data.code === 0) {
