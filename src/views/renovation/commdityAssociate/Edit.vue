<template>
  <el-dialog v-model="isVisible" :close-on-click-modal="false" :title="title" :type="type" center width="48%" top="50px"
    class="group-dialog">
    <div class="add-dialog-component">
      <el-alert :style="{ backgroundColor: '#fff' }" title="" type="info" :closable="false" />
      <el-tree ref="elTreeRef" class="tree-box" :data="treeData" :props="{ children: 'childs' }" node-key="id"
        default-expand-all :expand-on-click-node="false">
        <template #default="{ node, data }">
          <div class="custom-tree-node">
            <div class="content">
              <template v-if="data.depth < 4">
                <el-input v-model="data.categoryName" class="input" :disabled="isCheck || (data.depth === 1)"
                  maxlength="6" :placeholder="placeholderTips(data.depth)" />
              ...
              <template v-else>
                <div class="content" :class="isCheck ? 'disabled' : ''">
                  <el-input v-model="data.categoryName" class="input" :disabled="isCheck" maxlength="6"
                    placeholder="输入四级类别名称（最大6个字符）" />
                  <!-- <el-upload :headers="headers" :data="dataObj" :multiple="false" :show-file-list="false"
                    ...
                    :action="action">
                    <img v-if="data.categoryImgArray && data.categoryImgArray[0].url" width="80" height="80"
            ...
            <div v-if="!isCheck" class="setting-box">
              <el-button type="primary" link @click="() => append(data)">{{ addTips(data.depth) }}
              </el-button>
              <el-button type="primary" link @click="() => remove(node, data)" v-if="data.depth !== 1">删除
              </el-button>
            </div>
          </div>
        </template>
      ...
      <div class="add-btn-wrap">
        <template v-if="isCheck">
          <el-button class="add" type="primary" @click="close">确定
          </el-button>
        </template>
        <template v-else>
          <el-button v-if="type === 'add'" class="add" type="primary" @click="addClassification">添加一级类别名称
          </el-button>
...
<script setup>
import { commdityClassAdd, getChildren, updateAssociate } from '@/api/renovation';
