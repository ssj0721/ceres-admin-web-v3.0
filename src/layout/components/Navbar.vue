      ...
      <!-- 客服 -->
        ...
        >
          <router-link class="top-mag__menu"
            :to="{path:'/customer/yuanbao'}">
            <el-icon style="margin-right: 16px;margin-top: 14px; cursor: pointer;" size="25">
              <Service />
            </el-icon>
          </router-link>
        </el-tooltip>
        ...
        <el-popover placement="bottom" :width="440" trigger="hover">
          <template #reference>
              <el-icon style="margin-right: 16px;margin-top: 14px; cursor: pointer;" size="25" @mouseenter="getHistory">
                <el-badge is-dot><Bell /></el-badge>
              </el-icon>
          </template>
          <el-table :data="gridData">
            <el-table-column width="100" prop="noticeTitle" label="标题" />
            <el-table-column
              label="内容"
              width="150"
            >
              <template #default="scope">
                ...
                <el-table-column width="160" prop="createTime" label="时间" />
          </el-table>
          <router-link class="top-mag__menu"
                 :to="{path:'/notice/history'}">
            <el-button type="primary" link
                 size="small" style="float: right;">查看全部</el-button>
          </router-link>
        </el-popover>
...
import defaultAvatar from '@/assets/images/logo.webp'
...
import { noticeGetAll } from '@/api/notice'
...
const toggleSideBar = () => {
  console.log('点击toggleSideBar');
  store.commit('app/TOGGLE_SIDEBAR')
}


const getHistory = () => {
  console.log('获取历史消息执行了');
...
.navbar {
  ...
  overflow: hidden;
  position: relative;
  background: rgba($neutral-color-1,0.7);
  box-shadow:$shadow-1;
