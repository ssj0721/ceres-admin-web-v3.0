<template>
  <div>
    <!-- 导入补充资料 -->
    <el-dialog v-model="productImport" title="导入补充资料" :close-on-click-modal="false" center width="600px"
      @close="closeModal2">
...
import { importProduct2, matchCurDateForImportProduct } from '@/api/itemaudit/ait002'
...
const {productImport,uploading,batchFileList} = toRefs(prop);
const emits = defineEmits(['update:productImport','update:uploading','getAll']);
...
async function uploadFile2 () {
  ...
  if (batchFileList.value.length === 0) {
    ...
    let matchRes
    try {
      matchRes = await matchCurDateForImportProduct(formData)
      console.log('导入补充资料前校验年季、销售季期是否与当前日期匹配的res', matchRes)
    } catch (e) {
      uploading.value = false
      emits('update:uploading', false)
      ElMessage.error(e?.message || '导入前校验失败')
      return
    }
    ...
    importProduct2(formData).then((res) => {
      uploading.value = false
      emits("update:uploading", false)
      if(res.code=="" && res.data[0].code == 0){
        ...
        emits("update:productImport", false)
        batchFileList.value = []
        // getAll()
        emits('getAll');
